...
TEAMCITY_AGENT_OPTS_ACTUAL="$TEAMCITY_AGENT_OPTS -ea -XX:+DisableAttachMechanism --add-opens=java.base/java.lang=ALL-UNNAMED -XX:+IgnoreUnrecognizedVMOptions $TEAMCITY_AGENT_MEM_OPTS_ACTUAL -Dteamcity_logs=$LOG_DIR/"
...
if [ -f ../conf/teamcity-agent-log4j2.xml ]; then
    TEAMCITY_AGENT_OPTS_ACTUAL="$TEAMCITY_AGENT_OPTS_ACTUAL -Dlog4j2.configurationFile=file:../conf/teamcity-agent-log4j2.xml"
fi
...
need_java() {
    ...
    if [ -n "$TEAMCITY_JRE" ] && [ -n "$FORCE_USE_TEAMCITY_JRE" ]; then
      FJ_SKIP_ALL_EXCEPT_ARGS=1
      find_java 1.8 "$TEAMCITY_JRE" > /dev/null 2>&1
...
initial_configuration(){
    ...
    # Unset all stale JAVA_MAIN_CLASS_XXX environment variables TW-18725
    OLD_JAVA_MAIN=`env | sed -n 's/^\(JAVA_MAIN_CLASS_.*\)=.*/\1/p'`
    if [ "$OLD_JAVA_MAIN" != "" ]; then
      unset $OLD_JAVA_MAIN
    fi
}
...
start|run|probe)
        ...
        elif [ "$command_name" = "probe" ]; then
          "$java_exec" $TEAMCITY_LAUNCHER_OPTS_ACTUAL -cp $TEAMCITY_LAUNCHER_CLASSPATH jetbrains.buildServer.agent.Launcher $TEAMCITY_AGENT_OPTS_ACTUAL jetbrains.buildServer.agent.AgentMain -file $CONFIG_FILE -probe
        else
...
startExecutor)
    ...
    "$java_exec" -cp "../lib/*" jetbrains.buildServer.agent.AgentMain -file $CONFIG_FILE -startExecutor
    exit1 $?
    ;;
executor)
    initial_configuration
    ...
    "$java_exec" -cp "../lib/*" jetbrains.buildServer.agent.AgentMain -file $CONFIG_FILE -executor $BUILD_STEP
    exit1 $?
    ;;
publishParameters)
    initial_configuration
    ...
    "$java_exec" -cp "../lib/*" jetbrains.buildServer.agent.AgentMain -file $CONFIG_FILE -publishParameters $BUILD_STEP
    exit1 $?
    ;;
finishExecutor)
    initial_configuration

    "$java_exec" -cp "../lib/*" jetbrains.buildServer.agent.AgentMain -file $CONFIG_FILE -finishExecutor
    exit1 $?
    ;;
*)
        ...
        echo "  $this_script probe  - to start build agent only to probe configuration parameters and store them"
        echo "  $this_script publishParameters - to publish the details of the agent within this system back to the TeamCity server"
        echo "  $this_script startExecutor - to run all build start stages before the execution of the first container is reached (executor mode)"
        echo "  $this_script executor [BUILD_STEP] - to run a single build or build step and exit after that (executor mode)"
        echo "  $this_script finishExecutor - to run all build finish stages after the execution of last container is reached (executor mode)"
        exit1 1
        ;;
esac
